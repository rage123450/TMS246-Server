...
public class NpcShopItem {
    ...
    public void encode(OutPacket outPacket) {
        outPacket.encodeInt(getBuyLimit() == 0 ? -1 : getBuyLimit()); // -1 = infinite, 0 = nothing left, 100000
...
//        outPacket.encodeInt(getDiscountPerc());
        ...
        outPacket.encodeInt(0);
        outPacket.encodeInt(0);
        outPacket.encodeString("1900010100");
        outPacket.encodeString("2079010100");
/*        outPacket.encodeByte(getDiscountPerc());
        ...
        outPacket.encodeInt(0);
        outPacket.encodeInt(0);
        outPacket.encodeArr(new byte[40]); // decodeBuffer(32), npc id (4) + int (4) x4? Example id = 78 96 8F 00 (9410168)
        // end sub
    ...
    public FileTime getSellStart() {
        if (sellStart == null) {
            return FileTime.fromType(FileTime.Type.ZERO_TIME);
        }
        return sellStart;
    }
    ...
    public FileTime getSellEnd() {
        if (sellStart == null) {
            return FileTime.fromType(FileTime.Type.MAX_TIME);
        }
        return sellEnd;
    }
