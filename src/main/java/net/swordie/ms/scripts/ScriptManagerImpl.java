...
public class ScriptManagerImpl implements ScriptManager {
    ...
    public void setJob(short jobID) {
        ...
        chr.write(WvsContext.statChanged(stats, chr.getSubJob()));
    }
    ...
    public void changeCharacterLook(int look) {
        ...
        chr.getField().broadcastPacket(UserRemote.avatarModified(chr, maskValue, (byte) 0), chr); // broadcast look change to map
    }
    ...
    public void warpInstanceOutParty() { warpInstance(-1,false, 0, false); }
    ...
    @Override
    public void hasFieldProperty(String name) {
    }

    @Override
    public void setFieldProperty(String name, boolean bool) {
    }
    ...
    public Mob spawnAMob(int id, int x, int y, boolean respawnable, long hp) {
    ...
    public void pauseInstanceTime(boolean pause) {
        Instance instance = chr.getInstance();
        if (instance != null) {
            instance.pauseTimeout(pause);
        }
    }

    public int getInstanceRemainingTime() {
        Instance instance = chr.getInstance();
        return instance != null ? instance.getRemainingTime() : -1;
    }

    public void showTimerInfoEx(int seconds, int elapseTime) {
        chr.write(FieldPacket.clock(ClockPacket.TimerInfoEx(seconds, elapseTime)));
    }
    ...
    public boolean partyHasCoolDown(EventType type, int amountAllowed) {
        ...
        if (ServerConstants.RESTART_MINUTES > 0 && (Server.getInstance().MAINTENANCE_ACTIVE || Server.getInstance().MAINTENANCE_MODE || Server.getInstance().getUpTime() >= (ServerConstants.RESTART_MINUTES - 30) * 60000L))
            return true;
        if (chr.getParty() == null) {
            return true;
        }
...
//            return "server restarting soon";
    ...
    public void spawnLotus(byte phase, byte difficulty) {
        ...
        Mob lotus = spawnAMob(lotusId, 0, -16, false, hp);
        if (phase == 0) {
