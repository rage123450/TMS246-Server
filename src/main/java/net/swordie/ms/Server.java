...
public class Server extends Properties {
	...
	public void sendShutdownMessage(int time) {
		...
		getWorldById(ServerConfig.WORLD_ID.getVal()).broadcastPacket(UserLocal.chatMsg(ChatType.Notice2, "[Notice] " + msg + timeMsg + end));
		getWorldById(ServerConfig.WORLD_ID.getVal()).broadcastPacket(UserLocal.addPopupSay(9010063, 10000,
				"#e#b[Notice]#k#n " + msg + "#e#r" + timeMsg + "#k#n" + end, "FarmSE.img/boxResult"));
		ServerConfig.SLIDE_MSG = msg + timeMsg + end;
		getWorldById(ServerConfig.WORLD_ID.getVal()).broadcastPacket(WvsContext.broadcastMsg(BroadcastMsg.slideNotice(ServerConfig.SLIDE_MSG, true)));
	}
