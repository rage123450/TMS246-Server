...
public class GameConstants {
    ...
    public static final int NX_DROP_CHANCE = 1;
    ...
    public static final Set<Integer> STACKABLE_ITEM = new HashSet<>(Arrays.asList(
            2430182,
            2711003, 2711005, 2711012, 2711004, 2711006, 2711013, 05062000, 5062009, 5062010, 05062500, 5680479, 05062006, 05062002, 05062000,// In-Game Cubes
            2049153, 2049122, 2049116, 2049600, 2049610, 2046875, 2046876, 2046994, 2046995, 2047816, 2047817, 2047949, 2047950, 2047951, 2047952, 02612043, 02613042, 02615026, 02616057, 2048716, 2048717, 2048753, 2048755 // In-Game falme + scrolls
    ));
    ...
    public static final int FAME_COOLDOWN = 0; // in hours
    ...
    public static int[][][] INC_HP_MP = {
            ...
            {{30, 30}, {0, 0}, {0, 0}, {30, 30}, {0, 0}, {0, 0}}, // 17 - Demon Avenger
    ...
    private static void initQuickMove() {
        ...
        quickMoveInfos.add(new QuickMoveInfo(0, 9400301, QuickMoveType.Nurse, 0, "Style Room", false,
                FileTime.fromType(FileTime.Type.ZERO_TIME), FileTime.fromType(FileTime.Type.MAX_TIME)));
        quickMoveInfos.add(new QuickMoveInfo(0, 9000035, QuickMoveType.Boat, 1, "Boat", false,
                FileTime.fromType(FileTime.Type.ZERO_TIME), FileTime.fromType(FileTime.Type.MAX_TIME)));
    ...
    public static boolean isValidName(String name) {
        return name.length() >= 2 && name.length() <= 15/* && Util.isDigitLetterString(name)*/;
    }
