...
public class Mihile extends Job {
    ...
    private void giveSoulLinkBuffs() { // gives IndieDamR as well
        TemporaryStatManager tsm = chr.getTemporaryStatManager();
        if (chr != null && chr.hasSkill(SOUL_LINK) && tsm.hasStat(CharacterTemporaryStat.MichaelSoulLink)) {
            ...
            if (party != null) {
                ...
                for (Char partyChr : party.getPartyMembersInField(chr)) {
                    ...
                    if (partyChr.getId().equals(chr.getId()) || JobConstants.isMihile(partyChr.getJob())) {   // Mihile doesn't get the PartyMember's Buffs
                        continue;
                    }
                    ...
                    if (partyChrX >= rect.getLeft() && partyChrY >= rect.getTop()       // if  Party Members in Range
                        ...
                        partyTSM.putCharacterStatValue(CharacterTemporaryStat.MichaelSoulLink, o1);
                        ...
                        if (chr.getSkill(ROYAL_GUARD) != null && tsm.hasStatBySkillId(ROYAL_GUARD) && !partyTSM.hasStatBySkillId(ROYAL_GUARD)) {
                            ...
                            o2.tTerm = 12;
                            partyTSM.putCharacterStatValue(CharacterTemporaryStat.IndiePAD, o2);
                            partyTSM.putCharacterStatValue(CharacterTemporaryStat.IndieMAD, o2);
                        }
                        if (chr.getSkill(ENDURING_SPIRIT) != null && tsm.hasStatBySkillId(ENDURING_SPIRIT) && !partyTSM.hasStatBySkillId(ENDURING_SPIRIT)) {
                            ...
                            partyTSM.putCharacterStatValue(CharacterTemporaryStat.IndiePDDR, o3);
                            ...
                            partyTSM.putCharacterStatValue(CharacterTemporaryStat.IndieAsrR, o4);
                        }
            ...
            tsm.putCharacterStatValue(CharacterTemporaryStat.IndieDamR, o5);
            tsm.sendSetStatPacket();
        } else {
    ...
    public void soulLinkHPRegen(int slv) {
        TemporaryStatManager tsm = chr.getTemporaryStatManager();
        if (chr != null && chr.hasSkill(SOUL_LINK) && tsm.hasStat(CharacterTemporaryStat.MichaelSoulLink)) {
        ...
        switch (skillID) {
            ...
            case SOUL_LINK: // (ON/OFF)
                if (tsm.hasStatBySkillId(SOUL_LINK)) {
                    ...
                    tsm.putCharacterStatValue(CharacterTemporaryStat.MichaelSoulLink, o1);
                }
                if (soulLinkBuffsTimer != null && !soulLinkBuffsTimer.isDone()) {
                    soulLinkBuffsTimer.cancel(true);
                }
                soulLinkBuffsTimer = EventManager.addFixedRateEvent(this::giveSoulLinkBuffs, 0, 1, TimeUnit.SECONDS);

                if (soulLinkHPRegenTimer != null && !soulLinkHPRegenTimer.isDone()) {
                    soulLinkHPRegenTimer.cancel(true);
                }
                int delay = si.getValue(dot, slv);
                soulLinkHPRegenTimer = EventManager.addFixedRateEvent(() -> soulLinkHPRegen(slv), 0, delay, TimeUnit.SECONDS);
                break;
            case ROILING_SOUL:
