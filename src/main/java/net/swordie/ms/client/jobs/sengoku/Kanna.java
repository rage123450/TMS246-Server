...
public class Kanna extends Job {
    ...
    public void spawnHaku() {
        chr.write(UserPacket.skillPetCreated(chr, 1, HAKU));
    }
    ...
    private void spawnFoxMan(Option o1, int skillID, TemporaryStatManager tsm) {
        ...
        tsm.putCharacterStatValue(CharacterTemporaryStat.ChangeFoxMan, o1);
    ...
    public static void hakuFoxFire(Char chr) {
        ...
        o1.rOption = skillID;
        o1.tOption = si.getValue(SkillStat.time, slv);
        tsm.putCharacterStatValue(CharacterTemporaryStat.FireBarrier, o1);
    ...
    public static void hakuGift(Char chr) {
        if (!chr.hasSkillOnCooldown(HAKUS_GIFT_2) || !chr.hasSkillOnCooldown(HAKUS_GIFT)) {
            ...
            int healHP = (int) ((double) (chr.getMaxHP() * si.getValue(SkillStat.hp, 1)) / 100F);
            chr.heal(healHP);
    ...
    public static void hakuHakuBlessing(Char chr, Char other) {
        ...
        o1.rOption = skillID;
        o1.nOption = fan != null ? (int) (si.getValue(SkillStat.x, slv) * (fan.getiMad() / 100.0f)) : 0; // TODO: add to dmg calc?
        o1.tOption = 0;
        tsm.putCharacterStatValue(CharacterTemporaryStat.HakuBlessing, o1);
        tsm.sendSetStatPacket();
    }
    ...
    public static void hakuBreathUnseen(Char chr) {
        ...
        int gain = si.getValue(SkillStat.q, 1); // slv doesn't matter
    ...
    public void handleAttack(Client c, AttackInfo attackInfo) {
        ...
        if (chr.getField().getSummonByChrAndSkillId(chr, YUKI_MUSUME_SHOUKAN_SUMMON) != null
                ...
                && tsm.hasStat(CharacterTemporaryStat.YukiMusumeShoukan)
                && hasHitMobs) {
        ...
        switch (attackInfo.skillId) {
            ...
            case SHIKIGAMI_HAUNTING_4_3:
                ...
                for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                    ...
                    if (mob == null || !(Util.succeedProp(si.getValue(SkillStat.prop, slv)))) {
                        continue;
                    }
            ...
            case SOUL_SHEAR:
                ...
                break;
/*            case BURNING_SHIKIGAMI_ATTACK_COMBO:
                si = SkillData.getSkillInfoById(BURNING_SHIKIGAMI_DOT);
                slv = chr.getSkillLevel(BURNING_SHIKIGAMI_DOT);
                if (Util.succeedProp(si.getValue(prop, slv))) {
                    for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                        ...
                        MobTemporaryStat mts = mob.getTemporaryStat();
                        mts.createAndAddBurnedInfo(chr, BURNING_SHIKIGAMI_DOT, slv);
                    }
                }
                break;
            case FROZEN_SHIKIGAMI_ATTACK_COMBO:
                si = SkillData.getSkillInfoById(FROZEN_SHIKIGAMI_MS);
                slv = chr.getSkillLevel(FROZEN_SHIKIGAMI_MS);
                o1.nOption = -20;
                o1.rOption = FROZEN_SHIKIGAMI_MS;
                o1.tOption = 4;
                if (Util.succeedProp(si.getValue(prop, slv))) {
                    for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                        Mob mob = (Mob) chr.getField().getLifeByObjectID(mai.mobId);
            ...
            case FALLING_SAKURA:
                ...
                chrList.add(chr);
                int percentHealed = si.getValue(SkillStat.x, slv) + (chr.hasSkill(FALLING_SAKURA_VITALITY_H) ? 20 : 0);
            ...
            case YUKI_MUSUME_SHOUKAN:
                ...
                tsm.putCharacterStatValue(CharacterTemporaryStat.YukiMusumeShoukan, o2);
    ...
    private void spawnManaVein() {
        ...
        int maxManaVeins = si.getValue(SkillStat.x, slv) + chr.getSkillStatValue(SkillStat.x, GEOMANCY_SPREAD);
        if (getAmountOfManaVeins() < maxManaVeins && !chr.hasSkillOnCooldown(GEOMANCY)) {
            ...
            try {
                ...
                aa.setDuration(5000 + (mvsi.getValue(SkillStat.time, 1) + (chr.getSkillStatValue(SkillStat.time, GEOMANCY_PERSIST))) * 1000);
                ...
                chr.addSkillCoolTime(skill.getSkillId(), si.getValue(SkillStat.cooltime, slv) * 1000);
    ...
    private void yosuzume() {
        ...
        int chance = si.getValue(SkillStat.prop, slv);
    ...
    private void lifeBloodRitual(AttackInfo attackInfo) {
        ...
        SkillInfo si = SkillData.getSkillInfoById(skill.getSkillId());
        int slv = skill.getCurrentLevel();
        int healed = (int) ((chr.getMaxHP() * si.getValue(SkillStat.x, slv)) / 100D);
    ...
    private void doYukiMusumeShoukanAttack() {
        ...
        chr.heal((int) ((chr.getMaxHP() * si.getValue(SkillStat.y, slv)) / 100D));
    }

    private int getManaConsumptionBySkill(int skillId) {
        SkillInfo si = SkillData.getSkillInfoById(skillId);
        if (si != null) {
            return si.getValue(SkillStat.epCon, chr.getSkillLevel(skillId));
        }
        return 0;
    }
    ...
    private void incrementSpiritDomainCount() {
        ...
        SkillInfo si = SkillData.getSkillInfoById(skill.getSkillId());
        int slv = skill.getCurrentLevel();
        int stateIncReq = si.getValue(SkillStat.z, slv);
    ...
    public void handleSkill(Char chr, TemporaryStatManager tsm, int skillID, int slv, InPacket inPacket, SkillUseInfo skillUseInfo) {
        ...
        switch (skillID) {
            ...
            case MANA_BALANCE: // HP subtraction  done by client, this is only to server matches.
                chr.setStatAndSendPacket(Stat.hp, chr.getHPPerc(si.getValue(SkillStat.hp, slv)));
                break;
            ...
            case LIBERATED_SPIRIT_CIRCLE_SUMMON_2:
                ...
                int soulAmount = si.getValue((skillID == LIBERATED_SPIRIT_CIRCLE_SUMMON_2 ? SkillStat.u2 : SkillStat.u), slv);
            ...
            case BLACKHEARTED_CURSE:
                ...
                o1.rOption = skillID;
                o1.tOption = si.getValue(SkillStat.time, slv);
                tsm.putCharacterStatValue(CharacterTemporaryStat.BlackHeartedCurse, o1);
                break;
            case SPIRITS_DOMAIN:
                summon = Summon.getSummonByNoCTS(chr, skillID, slv);
                summon.setMoveAbility(MoveAbility.Stop);
            ...
            case SHIKIGAMI_DOPPLEGANGER:
                ...
                o2.rOption = skillID;
                o2.tOption = 0;
                tsm.putCharacterStatValue(Unk205_640, o2);
                break;
        }
        tsm.sendSetStatPacket();
    }
    ...
    public void spawnBlossomBarrier() {
        ...
        SkillInfo si = SkillData.getSkillInfoById(skill.getSkillId());
        int slv = skill.getCurrentLevel();
        int maxBarriers = chr.getSkillStatValue(SkillStat.x, WARDING_BARRIER);
    ...
    public void spawnBellflowerBarrier() {
        ...
        AffectedArea aa = AffectedArea.getPassiveAA(chr, skill.getSkillId(), slv);
        aa.setPosition(chr.getPosition());
        aa.setDuration((si.getValue(SkillStat.time, slv) + (chr.hasSkill(BELLFLOWER_BARRIER_PERSIST_H) ? 20 : 0)) * 1000);
    ...
    public void handleShootObj(Char chr, int skillId, int slv) {
        ...
        switch (skillId) {
            ...
            case LIBERATED_SPIRIT_CIRCLE_BIG:
                si = SkillData.getSkillInfoById(LIBERATED_SPIRIT_CIRCLE_SMALL);
                slv = chr.getSkillLevel(LIBERATED_SPIRIT_CIRCLE_SMALL);
                o1.nOption = skillId == LIBERATED_SPIRIT_CIRCLE_SMALL ? (tsm.hasStat(CharacterTemporaryStat.LiberatedSpiritCircle) ? tsm.getOption(CharacterTemporaryStat.LiberatedSpiritCircle).nOption + 1 : 1) : 0;
                o1.rOption = LIBERATED_SPIRIT_CIRCLE_SMALL;
                o1.xOption = skillId == LIBERATED_SPIRIT_CIRCLE_SMALL ? (tsm.hasStat(CharacterTemporaryStat.LiberatedSpiritCircle) ? tsm.getOption(CharacterTemporaryStat.LiberatedSpiritCircle).xOption + 1 : 1) : 0;
                tsm.putCharacterStatValue(CharacterTemporaryStat.LiberatedSpiritCircle, o1);
                tsm.sendSetStatPacket();
                chr.addSkillCoolTime(LIBERATED_SPIRIT_CIRCLE_SMALL, si.getValue(SkillStat.cooltime, slv) * 1000);
                chr.addSkillCoolTime(LIBERATED_SPIRIT_CIRCLE_BIG, si.getValue(SkillStat.cooltime, slv) * 1000);
                break;
    ...
    public void handleHit(Client c, InPacket inPacket, HitInfo hitInfo) {
        ...
        Option o = new Option();
        int foxfires = 6;
        if (tsm.hasStat(CharacterTemporaryStat.FireBarrier)) {
            if (foxfires > 1) {
                foxfires = foxfires - 1;
            }
            if (foxfires == 4 || foxfires == 3) {
                o.nOption = 2;
                tsm.putCharacterStatValue(CharacterTemporaryStat.FireBarrier, o);
                tsm.sendSetStatPacket();
            } else if (foxfires == 2) {
                o.nOption = 1;
                tsm.putCharacterStatValue(CharacterTemporaryStat.FireBarrier, o);
    ...
    public void resetFireBarrier() {
        TemporaryStatManager tsm = chr.getTemporaryStatManager();
        tsm.removeStat(CharacterTemporaryStat.FireBarrier, false);
        tsm.sendResetStatPacket();
    }
