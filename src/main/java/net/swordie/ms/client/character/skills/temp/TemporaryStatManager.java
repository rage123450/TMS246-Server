...
//                    summon.broadcastLeavePacket();
    ...
    public void encodeForLocal(OutPacket outPacket) {
        ...
        if (hasNewStat(FullSoulMP)) {
            outPacket.encodeInt(getOption(FullSoulMP).xOption);// local
        }
        short size = 0;
...
/*        if (hasNewStat(ZeroAuraStr)) {
        ...
        if (hasNewStat(Stance)) {
            outPacket.encodeInt(getOption(Stance).nOption); // 不確定
        }
        ...
        if (hasNewStat(DotHealHPPerSecond)) { // Unsure
        ...
        if (hasNewStat(SetBaseDamageByBuff)) {
            outPacket.encodeInt(getOption(SetBaseDamageByBuff).xOption);
        }
...
//        outPacket.encodeInt(getViperEnergyCharge());
        ...
        if (hasNewStat(BattlePvPHelenaMark)) {
            outPacket.encodeShort(getOption(BattlePvPHelenaMark).xOption);
        }
        if (hasNewStat(BattlePvPHelenaWindSpirit)) {
            outPacket.encodeShort(getOption(BattlePvPHelenaWindSpirit).xOption);
        }
        ...
        if (hasNewStat(VampDeath)) { // incorrect?
    ...
    public void encodeForRemote(OutPacket outPacket, Map<CharacterTemporaryStat, List<Option>> collection) {
        int[] mask = getMaskByCollectionRemote(collection);
        for (int maskElem : mask) {
            outPacket.encodeInt(maskElem);
        }

//        System.err.println("[encodeForRemote] " + collection.keySet().toString()); // TEMP REMOVE AFTER DEBUG XXX
//        System.err.println("[encodeForRemote] " + getNewStats().keySet().toString());
//        System.err.println("[encodeForRemote] " + getCurrentStats().keySet().toString());
        ...
        List<CharacterTemporaryStat> orderedAndFilteredCtsList = ctses.stream()
                ...
                .collect(Collectors.toList());
        for (CharacterTemporaryStat cts : orderedAndFilteredCtsList) {
            if (cts == FullSoulMP || cts == EnergyCharged || cts == WeaponCharge) {
                continue;
            }
                ...
                if (cts.isSeperatedDuplicate()) {
                    ...
                    switch (origin) {
                        case AntiMagicShellEx:
                            outPacket.encodeShort(originOpt.bOption);
                            outPacket.encodeInt(originOpt.rOption);
                            break;
                        case Unk199_521:
        ...
        if (ctsSet.contains(PoseType)) { // remote
        ...
        if (ctsSet.contains(BattlePvPHelenaMark)) {
            outPacket.encodeShort(collection.get(BattlePvPHelenaMark).get(0).xOption);
        }

        if (ctsSet.contains(BattlePvPHelenaWindSpirit)) {
            outPacket.encodeShort(collection.get(BattlePvPHelenaWindSpirit).get(0).xOption);
        }
    ...
    private void encodeIndieTempStat(OutPacket outPacket, Map<CharacterTemporaryStat, List<Option>> collection) {
        ...
        for (Map.Entry<CharacterTemporaryStat, List<Option>> stat : sortedStats.entrySet()) {
            ...
            for (Option option : options) {
                ...
                outPacket.encodeInt(0); // size for -> 下面的
    ...
    public void sendResetStatPacket(boolean demount) {
        if (getRemovedStats().size() > 0) {
            ...
            getChr().write(WvsContext.temporaryStatReset(this, demount));
        }
    }
