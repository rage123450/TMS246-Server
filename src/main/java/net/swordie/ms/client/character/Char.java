...
public class Char {
    ...
    @OneToOne(cascade = CascadeType.ALL, orphanRemoval = true)
    private Inventory consumeInventory = new Inventory(CONSUME, GameConstants.MAX_INV_SLOTS);

    @JoinColumn(name = "etcInventory")
    @OneToOne(cascade = CascadeType.ALL, orphanRemoval = true)
    private Inventory etcInventory = new Inventory(ETC, GameConstants.MAX_INV_SLOTS);

    @JoinColumn(name = "installInventory")
    @OneToOne(cascade = CascadeType.ALL, orphanRemoval = true)
    private Inventory installInventory = new Inventory(INSTALL, GameConstants.MAX_INV_SLOTS);

    @JoinColumn(name = "cashInventory")
    @OneToOne(cascade = CascadeType.ALL, orphanRemoval = true)
    private Inventory cashInventory = new Inventory(CASH, GameConstants.MAX_INV_SLOTS);

    @JoinColumn(name = "cashequipInventory")
    @OneToOne(cascade = CascadeType.ALL, orphanRemoval = true)
    private Inventory CashEquipInventory = new Inventory(CASH_EQUIP, GameConstants.MAX_INV_SLOTS);
    ...
    @OneToOne(cascade = CascadeType.ALL, orphanRemoval = true)
    private Inventory hairInventory = new Inventory(InvType.HAIR, 102/*3*/);

    @JoinColumn(name = "faceInventory")
    @OneToOne(cascade = CascadeType.ALL, orphanRemoval = true)
    private Inventory faceInventory = new Inventory(InvType.FACE, 102/*3*/);

    @JoinColumn(name = "skinInventory")
    @OneToOne(cascade = CascadeType.ALL, orphanRemoval = true)
    private Inventory skinInventory = new Inventory(InvType.SKIN, 102/*0*/);
...
//        monsterBattleRankInfo = new MonsterBattleRankInfo();
    ...
    public Inventory getCashEquipInventory() {
        return CashEquipInventory;
    }

    public void setCashEquipInventory(Inventory i) {
        this.CashEquipInventory = i;
    }
...
//        outPacket.encodeByte(0); // unk, not in kmst
        ...
        boolean b = false;
        outPacket.encodeByte(b); // again unsure
        if (b) {
...
/*        if (mask.isInMask(DBChar.ItemSlotConsume) || mask.isInMask(DBChar.ShopBuyLimit)) {
        ...
        if (mask.isInMask(DBChar.InventorySize)) {
            ...
            outPacket.encodeInt(getCashEquipInventory().getSlots());
        }
        ...
        if (mask.isInMask(DBChar.ItemSlotEquip)) {
            ...
            outPacket.encodeShort(0);//13

            // Android
            outPacket.encodeInt(0); // size-> 8+2+4+4+str+4+8
            ...
            outPacket.encodeShort(0);//16

            // CASH_EQUIP
            for (Item item : getCashEquipInventory().getItems()) {
                Equip equip = (Equip) item;
                outPacket.encodeShort(equip.getBagIndex());
                equip.encode(outPacket);
            }
            outPacket.encodeShort(0);//17

            outPacket.encodeShort(0);//18

            outPacket.encodeShort(0);//19

            outPacket.encodeShort(0);//20
        }
        ...
        if (mask.isInMask(DBChar.SkillRecord)) {
            boolean encodeSkills = true/*getSkills().size() > 0*/;
            ...
            for (LinkSkill linkSkill : linkSkills) { // sub
...
//			outPacket.encodeInt(getMonsterBookInfo().getSetID()); // removed 196
        }
        if (mask.isInMask(DBChar.QuestCompleteOld)) { // new 188
            // sub_C0D410
            short size = 0;
            outPacket.encodeShort(size);
            for (int i = 0; i < size; i++) {
        ...
        if (mask.isInMask(DBChar.NewYearCard)) { // new 196
            int size = 0;
            outPacket.encodeShort(size);
            for (int i = 0; i < size; i++) {
                outPacket.encodeInt(0);
        ...
        outPacket.encodeByte(bool123); // new 196
        if (bool123 && mask.isInMask(DBChar.Unk10000000000)) {
            int size = 0;
            outPacket.encodeInt(size);
            for (int i = 0; i < size; i++) {
                outPacket.encodeInt(0);
        ...
        if (mask.isInMask(DBChar.ZeroInfo)) { //v206
        ...
        if (mask.isInMask(DBChar.StolenSkills)) { // 64bytes
        ...
        if (mask.isInMask(DBChar.ChosenSkills)) { // 20bytes
            if (JobConstants.isPhantom(getAvatarData().getCharacterStat().getJob())) {
...
///////////////////////////////////////////////////////////////////////////////////
        ...
        outPacket.encodeShort(0); // size -> int*3 buffer 8*2 int int -> int
       ...
       //     getReturnEffectInfo().encode(outPacket); // ReturnEffectInfo::Decode
        ...
        if (mask.isInMask(DBChar.ActiveDamageSkin)) {
            ...
            outPacket.encodeFT(FileTime.fromType(FileTime.Type.ZERO_TIME)); // ftLastChanged?
...
/*        if (mask.isInMask(DBChar.FarmPotential)) {
        ...
        for (int i = 0; i < sizeO; i++) {
            ...
            outPacket.encodeString("Effect/Direction11.img/effect/meet/frame0/0"); // accept=0;date=22/09/08
        }
...
/*        if (mask.isInMask(DBChar.Unk400000)) {
    ...
    public Inventory getInventoryByType(InvType invType) {
        switch (invType) {
            ...
            case CASH_EQUIP:
                return getCashEquipInventory();
            case HAIR:
                return getHairInventory();
            case FACE:
                return getFaceInventory();
            case SKIN:
                return getSkinInventory();
            default:
                return null;
        }
    }
...
//        chatMessage("[addToBaseStat] id: %s", Integer.toString(si.getSkillId()));
    ...
    public void unequip(Item item) {
        ...
        if (isCash) {
            getInventoryByType(CASH_EQUIP).addItem(item);
        } else {
    ...
    public boolean equip(Item item, int oldPos, int newPos) {
        ...
        if (item.isCash()) {
            ...
            getInventoryByType(CASH_EQUIP).removeItem(item);
        }
...
//        chatScriptMessage(String.format("Hide is %s.", (hide ? "enabled" : "disabled")));
    ...
    public Inventory getSkinInventory() {
...
*/
    ...
    public void dispose() {
        ...
        write(WvsContext.statChangedEmpty());
    }
    ...
    public String SQL(String txtsql,Integer[] columnas){
        ...
        Query sql = session.createNativeQuery(txtsql);
        List<Object[]> results =sql.getResultList();

        for(Object[] r : results) {
            resultado+="[";
            for (Integer col: columnas) {
                resultado+= "'"+r[col].toString()+"',";
            }
            resultado+="],";
        }
        resultado+="]";
    ...
    public void Update(String Metodo, String Key, String Value, String KeyID, int ID) {
        Session session = DatabaseManager.getSession();
        Transaction transaction = session.beginTransaction();
        org.hibernate.query.Query sql = session.createNativeQuery( "UPDATE " + Metodo + " SET " + Key + " =  :Value WHERE " + KeyID + " = :ID ");
    ...
    private Char deepCopyForInvCheck() {
        ...
        chr.setCashEquipInventory(getCashEquipInventory().deepCopy());
        return chr;
    }
...
//                chatMessage("[addBaseStat else] key: %s value: %s", bs.toString(), Integer.toString(amount));
    ...
    public int getDojoPoints() {
        ...
        return Integer.parseInt(quest.getProperty("point"));
    }
    ...
    public void encodeChatInfo(OutPacket outPacket, String msg) {
            ...
            outPacket.encodeString(msg);
            outPacket.encodeInt(getUserId()); // 6F 56 72 6E
            outPacket.encodeInt(getId()); // 53 16 00 00
            outPacket.encodeByte(0); // 06
            outPacket.encodeInt(getId()); // 61 3F 98 01
            outPacket.encodeInt(0);
