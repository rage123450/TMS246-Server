...
public class AdminCommands {
    ...
    @Command(names = {"p"}, requiredType = Admin)
    public static class TestPacket_ extends AdminCommand {

        public static void execute(Char chr, String[] args) {
            if (args.length < 3) {
                chr.chatMessage("Usage: !p <op> <data>");
                return;
            }
            ...
            outPacket.encodeArr(new byte[90]);
            chr.write(outPacket);
        }
    }
    ...
    public static class TestPacket extends AdminCommand {
        ...
        public static void execute(Char chr, String[] args) {
            if (args.length < 4) {
                chr.chatMessage("Usage: !packet <op> <byte> <int> <byte>");
                return;
            }
            OutPacket outPacket = new OutPacket(Short.parseShort(args[1]));
...
/*    @Command(names = {"dc"}, requiredType = GameMaster)
    public static class dc extends AdminCommand {
        public static void execute(Char chr, String[] args) {
            Char victim = chr.getWorld().getCharByName((args[1]));
            if (victim != null) {
                victim.write(WvsContext.returnToTitle());
            } else {
...
//            party.addPartyMember(other);
    ...
    public static class AmountOnline extends AdminCommand {
        ...
        public static void execute(Char chr, String[] args) {
            ...
            chr.chatMessage(Notice, "Total users online: " + total);
        }
    }
