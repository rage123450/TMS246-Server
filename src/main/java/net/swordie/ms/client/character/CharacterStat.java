...
public class CharacterStat {
    ...
    public void gainPierce(double pierce) {
        if (this.pierce + pierce >= 100.00) {
            this.pierce = 100.00;
        } else this.pierce += pierce;
    }
    ...
    public void encode(OutPacket outPacket) {
        ...
        outPacket.encodeInt(0); // 相反的時間
        outPacket.encodeInt(0); // 相反的時間

        // Start burning info, names are guesses
        outPacket.encodeFT(FileTime.fromType(FileTime.Type.PLAIN_ZERO)); // ftStart
        outPacket.encodeFT(FileTime.fromType(FileTime.Type.ZERO_TIME)); // ftEnd
        outPacket.encodeInt(0);
        outPacket.encodeInt(0);
        outPacket.encodeInt(0);
