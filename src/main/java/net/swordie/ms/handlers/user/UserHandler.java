...
*/
    ...
    public static void handleUserSitRequest(Char chr, InPacket inPacket) {
        ...
        chr.setChair(new PortableChair(chr, 0));
    ...
    public static void handleUserPortableChairSitRequest(Char chr, InPacket inpacket) {
        ...
        if (textChair) {
            chair.setMsg(inpacket.decodeString()); // text to display
        }
        if (textChair && (chair.getMsg().length() > 12 || !Util.isValidString(chair.getMsg()) || !chr.hasItem(itemId))) {
            chr.chatMessage("Invalid text.");
            chr.dispose();
            return;
        }

        chair.addMeso(inpacket.decodeInt()); // meso

        boolean decodeLegionInfo = inpacket.decodeByte() != 0; // legion
        if (decodeLegionInfo) {
            inpacket.decodeInt(); // unk

            int loopSize = inpacket.decodeInt();
            for (int i = 0; i < loopSize; i++) {
                int level = inpacket.decodeInt(); // chrLv
                String name = inpacket.decodeString(); // chrName
                boolean encodeAL = inpacket.decodeByte() != 0; // encode 1st AL
                if (encodeAL) {
                    AvatarLook al = new AvatarLook();
                    al.decode(inpacket);
                }
                boolean encodeSubAL = inpacket.decodeByte() != 0; // encode Sub AL
                if (encodeSubAL) {
                    AvatarLook al = new AvatarLook();
                    al.decode(inpacket);
                }
                chair.addDisplayChrs(name);
            }
        }
    ...
    public static void handleRuneStoneUseRequest(Client c, InPacket inPacket) {
        Char chr = c.getChr();
        int runestoneSkillId = RuneStone.LIBERATED_RUNE_POWER;
        ...
        if (!chr.hasSkillOnCooldown(runestoneSkillId)) {
            ...
            if (minLevel > chr.getStat(Stat.level)) {
                chr.write(FieldPacket.runeStoneUseAck(RuneStoneAckType.TooStrongForYouToHandle));
                return;
            }

            // Send Arrow Message
            chr.write(FieldPacket.runeStoneUseAck(RuneStoneAckType.ShowArrows));
        } else {
            long minutes = (chr.getRemainingCoolTime(runestoneSkillId) / 60000);
            long seconds = (chr.getRemainingCoolTime(runestoneSkillId) / 1000);
    ...
    public static void handleRuneStoneSkillRequest(Client c, InPacket inPacket) {
        ...
        //field.broadcastPacket(FieldPacket.runeStoneSkillAck(runeStone.getRuneType()));
        runeStone.activateRuneStoneEffect(chr);
        chr.setRuneCooldown(System.currentTimeMillis());
        chr.addSkillCoolTime(RuneStone.LIBERATED_RUNE_POWER, GameConstants.RUNE_COOLDOWN_TIME * 60 * 1000);
        chr.dispose();
    }
