...
//                    totalDamage = (long) (totalDamage / 2 + (((totalDamage / 2) / 100) * chr.getAvatarData().getCharacterStat().getPierce()));
    ...
    public static void handleSummonedAttack(Client c, InPacket inPacket) {
        ...
        for (int i = 0; i < ai.mobCount; i++) {
            ...
            int idk7 = inPacket.decodeInt(); // 100
            short idk6 = inPacket.decodeShort(); // 990
            int idk8 = inPacket.decodeInt(); // 0
            int idk9 = inPacket.decodeInt(); // 0
...
/*        while (unk7 > 0 && unk7 < Short.MAX_VALUE) { // ?
        ...
        inPacket.decodeByte();
        inPacket.decodeByte();
        inPacket.decodeInt(); // A8 04 00 00
        inPacket.decodeInt(); // C7 01 00 00
        inPacket.decodeArr(14); // 都是00
        // END UNKNOWNATTACKPACKETDATA
...
//            short idk2 = inPacket.decodeShort();
        ...
        if (skillID == 25111005 || skillID == 23121002 || skillID == 23111002 || skillID == 23121003) { // Spirit Frenzy and spikes royal
        ...
        for (int i = 0; i < ai.mobCount; i++) {
            ...
            if (header == InHeader.USER_MAGIC_ATTACK) {
                mai.hpPerc = inPacket.decodeByte(); // 100
                if (skillID == 80001835) {
            ...
            parseAttackInfoPacket(inPacket, mai);
            inPacket.decodeInt(); // 66 02 AB 01
            inPacket.decodeArr(6); // 都是0
            inPacket.decodeInt(); // 4B FE CB CC
            ai.mobAttackInfo.add(mai);
        }
...
//            }
    ...
    private static void parseAttackInfoPacket(InPacket inPacket, MobAttackInfo mai) {
        ...
        mai.packetMakerUnk1 = inPacket.decodeByte(); // 01
