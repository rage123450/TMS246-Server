        ...
        switch (pt) {
            case PartyRes_ApplyParty_Accepted:
                ...
                if (!party.isFull()) {
                    ...
                    for (Char onChar : party.getOnlineMembers().stream().map(PartyMember::getChr).collect(Collectors.toList())) {
                        //onChar.write(WvsContext.partyResult(PartyResult.joinParty(party, chr.getName()))); // duplicated. @see addPartyMember
            ...
            case PartyRes_InviteIntrusion_Accepted:
                ...
                if (applier.getParty() != null) {
                    ...
                    /*for (Char onChar : party.getOnlineMembers().stream().map(PartyMember::getChr).collect(Collectors.toList())) {
                        onChar.write(WvsContext.partyResult(PartyResult.joinParty(party, applier.getName()))); // duplicated. @see addPartyMember
                    }*/
                } else {
