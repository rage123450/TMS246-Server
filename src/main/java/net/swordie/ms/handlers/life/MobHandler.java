...
//        msai.targetInfo = inPacket.decodeLong(); // contained in the following 3 decodes (2+2+4)
        ...
        boolean unk = inPacket.decodeInt() != 0;
        if (unk) {
            for (int i = 0; i < 11; i++) {
                inPacket.decodeInt();
            }
        ...
        // start new 199
        msai.idk1 = inPacket.decodeByte(); // 0A
        msai.ints = new int[10];
        for (int i = 0; i < 10; i++) {
            msai.ints[i] = inPacket.decodeInt();
            //	00000000(0)
            //	00000000(0)
            //	00000000(0)
            //	00000000(0)
        ...
        byte mask = inPacket.decodeByte(); // 00
        boolean targetUserIDFromSvr = (mask & 1) != 0;
        boolean isCheatMobMoveRand = ((mask >> 4) & 1) != 0;

        int hackedCode = inPacket.decodeInt(); // 01 00 00 00
        // if (xx = 18) inPacket.decodeString();
        int oneTimeActionCS = inPacket.decodeInt(); // CC DD FF 00
        int moveActionCS = inPacket.decodeInt(); // CC DD FF 00
        int hitExpire = inPacket.decodeInt(); // 0C 2A 53 4F
        inPacket.decodeInt(); // 00 00 00 00
        inPacket.decodeByte(); // 02
...
//                msai.targetInfo, msai.targetInfo, msai.skillID, msai.slv, msai.option));
        ...
        if (msai.skillID >= MobSkillID.PowerUp.getVal() && msai.skillID <= MobSkillID.No.getVal() && slv > 0
            ...
            if (mobSkill == null) {
             ...
             //   chr.chatMessage(ChatType.Mob, String.format("Mob " + mob + " did skill with ID %d (%s), level = %d", mobSkill.getSkillID(), MobSkillID.getMobSkillIDByVal(mobSkill.getSkillID()), mobSkill.getLevel()));mob.putSkillCooldown(skillID, slv, nextUseableTime);
