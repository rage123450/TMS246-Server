        ...
        switch (shr) {
            case Buy:
                ...
                if (chr.getEquipInventory().isFull() || chr.getConsumeInventory().isFull() || chr.getEtcInventory().isFull() || chr.getInstallInventory().isFull()) {
                    chr.write(ShopDlg.shopResult(new ShopResult(ShopResultType.FullInvMsg)));
                    chr.dispose();
                    return;
                }
                ...
                if (pos >= 0 && pos < trunk.getItems().size()) {
                    ...
                    if (chr.getInventoryByType(getItem.getInvType()).canPickUp(getItem)) {
                        ...
                        if (quantity <= getItem.getQuantity()) {
                            newquant = (short) (getItem.getQuantity() - quantity);
                            getItem.setQuantity(newquant);
                            if (newquant <= 0) {
