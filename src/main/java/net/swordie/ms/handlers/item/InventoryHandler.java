...
//        log.debug("Equip old: " + chr.getEquipInventory());
        ...
        InvType invTypeFrom = ((invType == EQUIP || invType == CASH_EQUIP) && oldPos < 0) ? EQUIPPED : invType;
        InvType invTypeTo = ((invType == EQUIP || invType == CASH_EQUIP) && newPos < 0) ? EQUIPPED : invType;
//        InvType invTypeFrom = invType == EQUIP ? oldPos < 0 ? EQUIPPED : EQUIP : invType;
//        InvType invTypeTo = invType == EQUIP ? oldPos < 0 ? EQUIPPED : EQUIP : invType;
...
//                log.debug("SwapItem before: " + swapItem);
            ...
            } else if ((invType == EQUIP || invType == CASH_EQUIP) && invTypeFrom != invTypeTo) {
                Inventory equippedInv = chr.getEquippedInventory();
                Inventory equipInv = item.isCash() ? chr.getCashEquipInventory() : chr.getEquipInventory();
...
//                    chr.chatMessage("Unequip");
                    ...
                    if (!chr.canEquip(item)) { // is this needed?
                        ...
                        chr.write(WvsContext.inventoryOperation(1, false, Collections.emptyList()));
                        return;
                    } else if (equipInv.isFull() && !item.isCash() && ItemConstants.isOverall(item.getItemId()) && equippedInv.getItemBySlot(BodyPart.Bottom.getVal()) != null) {
...
//                log.debug("Equip after: " + chr.getEquipInventory());
                ...
                if (afterSize + afterSizeOn != (beforeSize + beforeSizeOn + (invTypeTo == EQUIPPED ? 1 : 0))) {
                    log.error("afterSize: " + afterSize);
                    log.error("afterSizeOn: " + afterSizeOn);
                    log.error("beforeSize: " + beforeSize);
                    log.error("beforeSizeOn: "+ beforeSizeOn);
                    log.error("Data duplication!");
                }
            }
        }
